...
class PGLearner:
	def __init__(self, pa):
		...
		self.l_out = \
			build_newpg_network(pa.state_len, pa.state_width, pa.num_actions)
...
def build_newpg_network(input_height, input_width, output_length):
	...
	l_res = lasagne.layers.ReshapeLayer(l_in, ((-1,1,SUDOKU_SIZE, SUDOKU_SIZE**2)))

	l_conv = lasagne.layers.Conv2DLayer(l_res, 4, (1,4), stride=(1,4), pad='valid',W=lasagne.init.Normal(0.01),
		b=lasagne.init.Constant(0),nonlinearity=lasagne.nonlinearities.rectify)
	l_rowconv = lasagne.layers.Conv2DLayer(l_conv, 16, (1,4), stride=(1,4), pad='valid',W=lasagne.init.Normal(0.01),
		b=lasagne.init.Constant(0),nonlinearity=lasagne.nonlinearities.rectify)
	l_colconv = lasagne.layers.Conv2DLayer(l_conv, 16, (4,1), stride=(4,1), pad='valid',W=lasagne.init.Normal(0.01),
		b=lasagne.init.Constant(0),nonlinearity=lasagne.nonlinearities.rectify)
	l_boxconv = lasagne.layers.Conv2DLayer(l_conv, 16, (2,2), stride=(2,2), pad='valid',W=lasagne.init.Normal(0.01),
		b=lasagne.init.Constant(0),nonlinearity=lasagne.nonlinearities.rectify)
	l_rowflat = lasagne.layers.ReshapeLayer(l_rowconv, ((-1,SUDOKU_SIZE * CONV_WINDOWS ** 2)))
	l_colflat = lasagne.layers.ReshapeLayer(l_colconv, ((-1,SUDOKU_SIZE * CONV_WINDOWS ** 2)))
	l_boxflat = lasagne.layers.ReshapeLayer(l_boxconv, ((-1,SUDOKU_SIZE * CONV_WINDOWS ** 2)))
	l_all=lasagne.layers.ConcatLayer((l_rowflat, l_colflat, l_boxflat),axis=1)
